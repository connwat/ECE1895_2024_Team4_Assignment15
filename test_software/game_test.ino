...
void loop() {
  ...
  int time,
      wait      = 3000 - score*15,          // input period (time period where the user can respond to the beeps)
      between   = 500  - score*5;           // period between inputs (time before beeps of next iteration)

  print_score(score);                       // print score on console

  /* game loop                          decrement by 15ms and 5ms  */
  for(valid = true; valid && score < 99; wait -= 15, between -= 5){
